...
install: all
	mkdir -p $(BINDIR)
	cp -f spy_show spy spy_list_package_files spy_list_packages $(BINDIR)
	mkdir -p $(MANDIR)
	cp -f spy.1 $(MANDIR)
	mkdir -p $(ETCDIR)/spy
...
uninstall:
	rm -f $(BINDIR)/spy $(BINDIR)/spy_show $(BINDIR)/spy_list_package_files $(BINDIR)/spy_list_packages
	rm -f $(MANDIR)/spy.1
	rm -f $(ETCDIR)/spy/update
	rmdir $(ETCDIR)/spy

clean:
	rm -f spy spy.1 spy_show
	$(GO) clean
